...
class Action
{
	...
	function save_employee()
	{
		...
		$checkQuery = $this->db->query("SELECT COUNT(*) as count FROM employee_regular WHERE fullname = '$fullname' " . (empty($id) ? "" : " AND id != $id"));
		...
		$data = "fullname='$fullname', position='$position', employee_id='$employee_id'";

		if (empty($id)) {
			$save = $this->db->query("INSERT INTO employee_regular SET " . $data);
		} else {
			$save = $this->db->query("UPDATE employee_regular SET " . $data . " WHERE id = " . $id);
		}

		if ($save) {
			return 1;
		}
		return "Error: Unable to save employee data.";
	}

	function save_parttime()
	{

		extract($_POST);

		// Check if fullname already exists
		$checkQuery = $this->db->query("SELECT COUNT(*) as count FROM employee_parttime WHERE fullname = '$fullname' " . (empty($id) ? "" : " AND id != $id"));
		$result = $checkQuery->fetch_assoc();
		...
		$data = "fullname='$fullname', position='$position', category='$category'";

		if (empty($id)) {
			$save = $this->db->query("INSERT INTO employee_parttime SET " . $data);
		} else {
			$save = $this->db->query("UPDATE employee_parttime SET " . $data . " WHERE id = " . $id);
		}

		if ($save) {
			return 1;
		}
		return "Error: Unable to save part-time employee data.";
	}
	...
	function save_cos()
	{
		...
		$checkQuery = $this->db->query("SELECT COUNT(*) as count FROM employee_cos WHERE fullname = '$fullname' " . (empty($id) ? "" : " AND id != $id"));
		$result = $checkQuery->fetch_assoc();
		...
		$data = "fullname='$fullname', position='$position'";

		if (empty($id)) {
			$save = $this->db->query("INSERT INTO employee_cos SET " . $data);
		} else {
			$save = $this->db->query("UPDATE employee_cos SET " . $data . " WHERE id = " . $id);
		}

		if ($save) {
			return 1;
		}
		return "Error: Unable to save COS employee data.";
	}
	...
	function save_wages()
	{
		...
		$data .= ", pera='$pera' ";
		$data .= ", gross_amount_earned='$gross_amount_earned' ";
		$data .= ", pagibig_ps='$pagibig_ps' ";
		$data .= ", pagibig_gs='$pagibig_gs' ";
		$data .= ", pagibig_mp3='$pagibig_mp3' ";
		$data .= ", gsis_ps='$gsis_ps' ";
		$data .= ", gsis_gs='$gsis_gs' ";
		$data .= ", sif='$sif' ";
		$data .= ", philhealth_ps='$philhealth_ps' ";
		$data .= ", philhealth_gs='$philhealth_gs' ";
		...
		$data .= ", cfi='$cfi' ";
		$data .= ", csb='$csb' ";
		$data .= ", total_deductions='$total_deduction' ";
		$data .= ", net_salary='$net_salary' ";
		$data .= ", net_received1='$net_received1' ";
		$data .= ", net_received2='$net_received2' ";

		if (empty($id)) {
			$save = $this->db->query(" INSERT INTO wages set " . $data);
		} else {
			$save = $this->db->query(" UPDATE wages set " . $data . " where id = " . $id);
		}
		if ($save) {
			return 1;
		}
	...
	function save_wages_parttime()
	{
		extract($_POST);
		$data = " employee_id='$employee_id' ";
		$data .= ", hrs_work='$hrs_work' "; // Added for hours worked
		$data .= ", rate_per_hr='$rate_per_hr' "; // Added for rate per hour
		$data .= ", underpayment='$underpayment' "; // Added for underpayment
		$data .= ", overtime='$overtime' "; // Added for overtime
		$data .= ", grand_total='$grand_total' ";
		$data .= ", total_deduction='$total_deduction' "; // Retaining total deductions
		$data .= ", net_amount='$net_amount' "; // Updated to use net amount due


		if (empty($id)) {
			$save = $this->db->query("INSERT INTO wages_parttime SET " . $data);
		} else {
			$save = $this->db->query("UPDATE wages_parttime SET " . $data . " WHERE id = " . $id);
		}
	...
	function save_wages_cos()
	{
		...
		$data = " employee_id='$employee_id' ";
		$data .= ", days_work='$days_work' "; // Days worked
		$data .= ", rate_per_hour='$rate_per_hour' "; // Rate per hour
		$data .= ", tardiness='$tardiness' "; // Tardiness
		$data .= ", overtime='$overtime' "; // Overtime
		$data .= ", undertime='$undertime' "; // Undertime
		$data .= ", sss='$sss' "; // SSS contribution
		$data .= ", total_deduction='$total_deduction' "; // Total deductions
		$data .= ", net_amount='$net_amount' "; // Net amount due

		// Check if ID is empty for INSERT or not for UPDATE
		if (empty($id)) {
			$save = $this->db->query("INSERT INTO wages_cos SET " . $data);
		} else {
			$save = $this->db->query("UPDATE wages_cos SET " . $data . " WHERE id = " . $id);
		}
	...
	function get_wages_parttime()
	{

		extract($_POST);


		$query = $this->db->query("SELECT * FROM wages_parttime WHERE id = '$id'");

		if ($query->num_rows > 0) {

			$wage = $query->fetch_assoc();

			return json_encode($wage);
		} else {
	...
	function get_wage()
	{
		...
		$query = $this->db->query("SELECT * FROM wages WHERE id = '$id'");
	...
	function get_wages_cos()
	{

		extract($_GET);


		$query = $this->db->query("SELECT * FROM wages_cos WHERE id = '$id'");
